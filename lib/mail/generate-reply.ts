...
async function callModel(input: string) {
  ...
  try {
    const response = await fetch("https://ai-gateway.vercel.sh/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "Geef uitsluitend JSON: subject, body, confidence, confidenceReasons, missingInformation.",
        ].join(" "),
        input,
    ...
    if (!text) return null
    return responseSchema.safeParse(JSON.parse(text.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "")))
  } catch {
    return null
  } finally {
...
export async function generateReplyDraft(supabase: SupabaseClient, input: { threadId: string; messageId: string; force?: boolean }) {
  if (!input.force) {
  ...
  try {
    ...
    const generated = modelResult?.success ? modelResult.data : fallback
    ...
    const { data: saved, error } = await supabase.from("mail_drafts").update({
      ...
      model: modelResult?.success ? MODEL : null,
