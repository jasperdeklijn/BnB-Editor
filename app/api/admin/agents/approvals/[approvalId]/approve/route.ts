...
import { z } from "zod"
import { executeAgentExecution } from "@/lib/agents/executor"
...
export async function POST(request: Request, context: { params: Promise<{ approvalId: string }> }) {
  ...
  const { data, error } = await admin.rpc("decide_agent_approval", { p_approval_id: approvalId, p_decision: "approve", p_decided_by: auth.user.id, p_note: parsed.data.note ?? null })
  if (error) return NextResponse.json({ error: error.message }, { status: 409 })
  const decision = Array.isArray(data) ? data[0] : data
  let execution = null
  if (decision?.execution_id) {
    try {
      execution = await executeAgentExecution(admin, decision.execution_id)
    } catch {
      execution = { executionId: decision.execution_id, status: "pending" }
    }
  }
