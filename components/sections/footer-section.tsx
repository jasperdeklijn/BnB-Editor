      ...
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${layout.section} ${layout.layout === "compact" ? "max-w-5xl" : "max-w-7xl"}`}>
        <div className={`mb-8 grid gap-8 ${showLinks || hasCompanyInfo ? "md:grid-cols-3" : ""}`}>
          <div className={layout.layout === "banner" ? "text-center md:col-span-3" : ""}>
            <EditableText as="h3" data={editableData} path={["companyName"]} value={companyName} isPreview={isPreview} onUpdate={onUpdate} className="mb-3 text-lg font-bold text-[var(--section-accent)]" />
          ...
          {showLinks ? <div className={hasCompanyInfo ? "" : "md:col-span-2"}>
            {columns.map((column, columnIndex) => <div key={columnIndex}>
              ...
              <ul className="flex flex-wrap gap-x-5 gap-y-2 md:flex-col">
                {column.links.map((link, linkIndex) => <li key={`${link.href}-${linkIndex}`}><Link href={resolveHref(link.href)} className="text-sm opacity-75 transition hover:text-[var(--section-accent)] hover:opacity-100"><EditableText data={editableData} path={["columns", columnIndex, "links", linkIndex, "label"]} value={link.label} isPreview={isPreview} onUpdate={onUpdate} /></Link></li>)}
              </ul>
            </div>)}
          </div> : null}
        </div>
        ...
        <div className="border-t border-white/20 pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left">
            ...
            {showSocialLinks ? <div className="flex flex-wrap justify-center gap-4">{socialLinks.map((link, index) => <a key={`${link.href}-${index}`} href={link.href} className="text-sm opacity-75 transition hover:text-[var(--section-accent)] hover:opacity-100"><EditableText data={editableData} path={["socialLinks", index, "label"]} value={link.label} isPreview={isPreview} onUpdate={onUpdate} /></a>)}</div> : null}
          </div>
        </div>
      </div>
    </footer>
