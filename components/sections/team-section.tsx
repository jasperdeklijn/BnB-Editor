        ...
        {members.length === 0 ? (
          ...
          <div className={isHorizontal ? "flex snap-x gap-5 overflow-x-auto pb-3" : isList ? "grid gap-5 md:grid-cols-2" : `grid gap-6 ${layout.grid}`}>
            {members.map((member, index) => (
              <article
                key={member.id ?? index}
                className={`${isHorizontal ? "min-w-[260px] snap-start" : ""} ${isList ? "flex items-center gap-4" : "text-center"} overflow-hidden rounded-2xl border border-border p-5 shadow-sm`}
                style={{ backgroundColor: styles?.surfaceColor || "rgba(255,255,255,0.85)" }}
              >
                ...
                <div className={isList ? "min-w-0 text-left" : "mt-4"}>
                  ...
                  {member.title ? <EditableText data={data} path={["members", index, "title"]} value={member.title} isPreview={isPreview} onUpdate={onUpdate} className="mt-1 text-sm font-medium" style={{ color: styles?.accentColor || "#b45309" }} /> : null}
