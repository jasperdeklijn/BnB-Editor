...
export function ReviewForm({ websiteId, available, data = {}, styles, preview = false }: ReviewFormProps) {
  ...
  return <div className={`${getReviewPanelClass(data.styleType)} space-y-6 [overflow-wrap:anywhere] ${styles?.fontFamily ?? ""}`} style={getSectionColorVars(styles, { accent: "#385344", surface: "#ffffff" })}
    onClick={preview ? (event) => event.stopPropagation() : undefined} onKeyDown={preview ? (event) => event.stopPropagation() : undefined}>
    {preview && <p className="text-xs opacity-75">Voorbeeldformulier · inzendingen worden niet verstuurd</p>}
    ...
    {available ? <form onSubmit={submit} className="space-y-5">
      <div><h2 className="text-xl font-semibold sm:text-2xl">{settings.reviewFormTitle}</h2><p className="mt-2 text-sm opacity-75">{settings.reviewFormIntro}</p></div>
      <div className={data.layout === "compact" ? "space-y-4" : "grid gap-4 sm:grid-cols-2"}>
        <div className="min-w-0 space-y-1.5"><Label htmlFor={`${prefix}-name`}>{settings.reviewNameLabel}</Label><Input className={inputClass} id={`${prefix}-name`} name="name" required maxLength={120} autoComplete="name" /></div>
        <div className="min-w-0 space-y-1.5"><Label htmlFor={`${prefix}-email`}>{settings.reviewEmailLabel}</Label><Input className={inputClass} id={`${prefix}-email`} name="email" type="email" required maxLength={254} autoComplete="email" /></div>
      </div>
      <fieldset className="space-y-2" onMouseLeave={() => setHoverRating(0)}>
        <legend className="text-sm font-medium">{settings.reviewRatingLabel}</legend>
      ...
      <Button type="submit" disabled={busy || preview} className="h-auto min-h-11 w-full whitespace-normal bg-[var(--section-accent)] py-3 text-[var(--section-accent-foreground)] hover:bg-[var(--section-accent)] hover:opacity-90">{busy ? "Bezig…" : settings.reviewSubmitLabel}</Button>
    ...
    <details className="border-t border-current/20 pt-4">
      ...
      <form onSubmit={requestEmail} className="mt-4 space-y-3">
        ...
        <Button type="submit" variant="outline" disabled={busy || preview} className="w-full text-foreground">Link aanvragen</Button>
      </form>
    </details>
