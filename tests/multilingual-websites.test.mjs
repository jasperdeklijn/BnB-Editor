...
test("schema, routing, and public selector include multilingual safeguards", () => {
  ...
  const deletionRepair = fs.readFileSync(path.resolve("supabase/migrations/20260729120000_repair_multilingual_website_deletion.sql"), "utf8")
  const bootstrap = fs.readFileSync(path.resolve("supabase/init.sql"), "utf8")
  ...
  assert.match(deletionRepair, /pg_trigger_depth\(\) = 1/)
  assert.match(bootstrap, /pg_trigger_depth\(\) = 1/)
